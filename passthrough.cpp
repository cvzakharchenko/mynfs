...
void log_str(std::string s)
{
	std::string path = "/home/cvz/pro/nfs/mynfs/run.log";
	std::ofstream log_file(path, std::ofstream::app);
	log_file << s << std::endl;
	log_file.close();
}

std::string map_path(std::string path)
{
	static const std::string root_dir = "/home/cvz/pro/nfs/mynfs/local";
	const std:: string new_path = root_dir + path;
...
{
	std::string new_path = map_path(path);
	path = new_path.c_str();
